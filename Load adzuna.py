...
SALIDA_DEFAULT    = Path("data/processed/adzuna_sample.csv")

# Países con habla inglesa (para orientar al usuario sobre --idioma)
...
[3/3] Listo. Próximo paso — extrae las skills:
  ...
  Puedes combinar fuentes creando un CSV unificado:

    python3 load_adzuna.py --pais co --muestra 5000   # mercado colombiano
    python3 load_adzuna.py --pais gb --muestra 5000   # benchmark Reino Unido
    python3 extract_skills.py adzuna_sample.csv descripcion id_oferta --idioma {idioma}

  Países soportados:
    co (Colombia), gb (UK), us (EE.UU.), au (Australia),
    de (Alemania), fr (Francia), ca (Canadá), br (Brasil),
    in (India), nl (Países Bajos), nz (Nueva Zelanda),
    pl (Polonia), ru (Rusia), sg (Singapur), za (Sudáfrica)
""")
...
def parse_args():
    ...
    parser.add_argument("--pais",    default="co",   help="Código de país (default: co)")
    parser.add_argument("--query",   default="",     help="Término de búsqueda (default: todas)")
    parser.add_argument("--muestra", default=5000, type=int, help="Número de ofertas (default: 5000)")
    parser.add_argument("--salida",  default=str(SALIDA_DEFAULT), help="Ruta CSV de salida")
...
if __name__ == "__main__":
    ...
    df = cargar_adzuna(pais=args.pais, query=args.query, muestra=args.muestra)
    guardar(df, salida)
    instrucciones(args.pais, salida)